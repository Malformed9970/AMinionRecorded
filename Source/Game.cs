...
public static unsafe class Game
{
    ...
    private static bool pendingPauseAfterChapterLoad = false;
    private static float? pendingSeekAfterChapterLoad = null;

    private static readonly HashSet<uint> whitelistedContentTypes = [ 1, 2, 3, 4, 5, 9, 28, 29, 30, 37, 39 ]; // 22 Event, 26 Eureka, 27 Carnivale
    ...
    private static void PlaybackUpdateDetour(ContentsReplayModule* contentsReplayModule)
    {
        ...
        if (contentsReplayModule->InPlayback)
        {
            if (contentsReplayModule->IsLoadingChapter)
            {
                if (ARealmRecorded.Config.AutoPauseAfterChapterSelect)
                    pendingPauseAfterChapterLoad = true;
            }
            else if (pendingPauseAfterChapterLoad || pendingSeekAfterChapterLoad.HasValue)
            {
                if (pendingSeekAfterChapterLoad.HasValue)
                {
                    contentsReplayModule->seek = pendingSeekAfterChapterLoad.Value;
                    pendingSeekAfterChapterLoad = null;
                }

                if (pendingPauseAfterChapterLoad)
                {
                    contentsReplayModule->playbackControls |= 8;
                    pendingPauseAfterChapterLoad = false;
                }
            }
        }
    ...
    {
        // FFXIV Natively permits clicking the currently active chapter to restart it. 
        // Doing so triggers a dynamic chronological rollback (timeline scrubbing) without a Zone Unload (Loading Screen).
        // External C++ packet parsers categorically do not support dynamic chronological scrubbing backwards
        // inside an active zone instance, and will crash the engine instantly with Access Violation 0x12345679.
        var isSameChapter = chapter == contentsReplayModule->GetCurrentChapter() + 1;
        var originalSeek = contentsReplayModule->seek;
    ...
    public static void JumpToChapter(byte chapter, float? seek = null)
    {
        var contentsReplayModule = Common.ContentsReplayModule;
        if (contentsReplayModule == null || !contentsReplayModule->InPlayback) return;

        pendingSeekAfterChapterLoad = seek;
        var currentChapter = contentsReplayModule->GetCurrentChapter();
        var targetChapter = (byte)(chapter + 1);

        if (targetChapter == currentChapter + 1)
        {
            var fakeSeek = targetChapter == 1 ? (contentsReplayModule->chapters[1]->ms / 1000f) : 0f;
            contentsReplayModule->seek = fakeSeek;
        }

        contentsReplayModule->SetChapter(targetChapter);
    }
    ...
    private static Bool ReplayPacketDetour(ContentsReplayModule* contentsReplayModule, FFXIVReplay.DataSegment* segment, byte* data)
    {
        try
        {
            ...
            return ReplayPacketManager.ReplayPacket(segment, data) || ContentsReplayModule.replayPacket.Original(contentsReplayModule, segment, data);
        }
        catch (Exception e)
        {
            DalamudApi.LogError($"Error replaying packet: {e}");
            return false;
        }
    }
    ...
    public static FFXIVReplay* ReadReplay(string path)
    {
        ...
        try
        {
            ...
            var allocSize = (int)fs.Length;
            ptr = Marshal.AllocHGlobal(allocSize + 0x1000);
            allocated = true;

            var span = new Span<byte>((void*)ptr, allocSize + 0x1000);
            span.Clear();
            _ = fs.Read(span[..allocSize]);
        }
        catch (Exception e)
        {
