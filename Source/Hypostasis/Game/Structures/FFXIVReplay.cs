...
public unsafe partial struct FFXIVReplay
{
    ...
    public DataSegment* GetDataSegment(uint offset)
    {
        var dataSize = header.replayLength - sizeof(Header) - sizeof(ChapterArray);
        if (offset + sizeof(DataSegment) > dataSize)
            return null;

        var segment = (DataSegment*)(Data + offset);
        if (offset + segment->Length > dataSize)
            return null;
